...
const api = (m, current) => {
  ...
  const build = () => {
    ...
    for (let i = 1; i < 11; i++) {
      const p = pkb(seeds[i]);
      const key = current ? m.Key.from(p.ed25519PublicKeys(), { privateKeys: p.ed25519PrivateKeys() }) : m.Key.newWithPrivateKeys(p.ed25519PrivateKeys(), p.ed25519PublicKeys());
      doc.addKey(key);
    }
    return doc;
  };
  const sign = (doc) => (current ? doc.toEnvelope({ sign: "inception" }) : doc.toEnvelope(m.XIDPrivateKeyOptions.Omit, m.XIDGeneratorOptions.Omit, { type: "inception" }));
  const verify = (env) => (current ? m.XIDDocument.fromEnvelope(env, { verify: "inception" }) : m.XIDDocument.fromEnvelope(env, undefined, m.XIDVerifySignature.Inception));
  const parse = (env) => (current ? m.XIDDocument.fromEnvelope(env) : m.XIDDocument.fromEnvelope(env));
