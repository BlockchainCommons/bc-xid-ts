...
function makeFakePrivateKeyBase(): PrivateKeyBase {
  ...
  return PrivateKeyBase.random({ rng: rng });
}
...
function makeFakePrivateKeyBases(count: number): PrivateKeyBase[] {
  ...
  for (let i = 0; i < count; i++) {
    result.push(PrivateKeyBase.random({ rng: rng }));
  }
  return result;
}
...
describe("XID Document Test Vectors", () => {
  describe("Basic XID Document", () => {
    it("should create XID document with deterministic key and verify XID format", () => {
      ...
      const xidUrString = xid.toUR().toString();
      expect(xidUrString).toMatch(/^ur:xid\//);
      const xid2 = decodeURWith(UR.parse(xidUrString), XID.codec);
  ...
  describe("Minimal XID Document", () => {
    it("should create minimal XID document from XID only", () => {
      ...
      const tempDoc = XIDDocument.from({ inceptionKey: privateKeyBase.ed25519PublicKeys() });
  ...
  describe("Key Change", () => {
    it("should allow removing inception key and adding new key", () => {
      ...
      const xidDocument = XIDDocument.from({ inceptionKey: privateKeyBase1 });
      ...
      // Add new key
      const newKey = Key.allowAll(privateKeyBase2.ed25519PublicKeys());
  ...
  describe("Service with References", () => {
    it("should create service with key and delegate references", () => {
      ...
      const aliceDoc = XIDDocument.from({ inceptionKey: aliceBase.ed25519PublicKeys() });
      const aliceKey = aliceDoc.inceptionKey;
      if (aliceKey === undefined) throw new Error("Expected inception key");

      // Create Bob's document
      const bobDoc = XIDDocument.from({ inceptionKey: bobBase.ed25519PublicKeys() });

      // Create Bob as delegate with permissions
      const bobDelegate = Delegate.from(bobDoc);
      ...
      // Round-trip
      const envelope = aliceDoc.toEnvelope();
...
describe("Signing Options Tests", () => {
  ...
  describe("SigningOptions.PrivateKeys", () => {
    ...
    it("should sign with provided private keys", () => {
      ...
      const xidDocument = XIDDocument.from({ inceptionKey: docKeyBase });
...
describe("XID Identifier Format Tests", () => {
  describe("XID UR Encoding", () => {
    it("should produce valid UR string", () => {
      ...
      // Round-trip
      const xid2 = decodeURWith(UR.parse(urString), XID.codec);
      expect(xid.equals(xid2)).toBe(true);
    });
  });
...
describe("XID CBOR Hex Verification", () => {
  ...
  it("should produce correct XID CBOR structure (tag 40024 + 32 bytes)", () => {
    ...
    const cborData = xid.toCbor().toData();
  ...
  it("should produce correct CBOR diagnostic notation", () => {
    ...
    const taggedCbor = xid.toCbor();
    const diag = diagnostic(taggedCbor);

    // Should contain tag 40024 and the XID bytes
    expect(diag).toContain("40024");
    expect(diag.toLowerCase()).toContain(xidHex.toLowerCase());
  });
...
describe("XID Known Test Vector (Rust Parity)", () => {
  ...
  it("should produce correct CBOR diagnostic notation for known XID", () => {
    const xid = XID.fromHex(TEST_XID_HEX);
    const diag = diagnostic(xid.toCbor());

    // From Rust: 40024(h'71274df133169a0e2d2ffb11cbc7917732acafa31989f685cca6cb69d473b93c')
    expect(diag).toContain("40024");
    expect(diag.toLowerCase()).toContain(TEST_XID_HEX.toLowerCase());
  });
});
...
describe("XID Document Cross-Impl Vectors", () => {
  ...
  //     `@blockchaincommons/rand`. `PrivateKeyBase::new_using(rng)` consumes 32 bytes;
  ...
  it("produces XID with byte-identical hex to Rust", () => {
    ...
    const doc = XIDDocument.from({ inceptionKey: publicKeys });
  ...
  it("produces a byte-identical `ur:xid/...` string to Rust", () => {
    ...
    const doc = XIDDocument.from({ inceptionKey: publicKeys });
    const ur = doc.toUR().toString();
    expect(ur).toBe(RUST_XID_DOCUMENT_UR);

    // Round-trip: decode the Rust-produced UR, re-encode, compare.
    const doc2 = XIDDocument.fromUR(UR.parse(RUST_XID_DOCUMENT_UR));
    expect(doc2.toUR().toString()).toBe(RUST_XID_DOCUMENT_UR);
  ...
  it("empty XIDDocument UR uses raw 32-byte body, matching Rust", () => {
    ...
    const privateKeyBase = PrivateKeyBase.random({ rng: rng });
    const publicKeys = privateKeyBase.schnorrPublicKeys();
    const fullDoc = XIDDocument.from({ inceptionKey: publicKeys });
    const emptyDoc = XIDDocument.fromXid(fullDoc.xid);

    expect(emptyDoc.toUR().toString()).toBe(RUST_EMPTY_XID_UR);

    const decoded = XIDDocument.fromUR(UR.parse(RUST_EMPTY_XID_UR));
    expect(decoded.isEmpty).toBe(true);
    expect(decoded.xid.equals(emptyDoc.xid)).toBe(true);
    expect(decoded.toUR().toString()).toBe(RUST_EMPTY_XID_UR);
  });
});
